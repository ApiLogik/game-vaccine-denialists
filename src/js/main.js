const onLoadGlobal = () => {
	...
	const preLoadImages = () => {
		...
		for (let i = 0; i <= 2; i++) {
			...
			if(document.documentElement.lang === 'pt-BR') {
				...
				loadedImages[1][i].src = `assets/media/negac${i+1}_hit_eng.png`;
			}
		}
	...
	const showResult = (counter, level) => {
		...
		if(document.documentElement.lang === 'pt-BR') {
			level === 'easy' ? showLevel.innerHTML = 'fácil' : false;
			level === 'normal' ? showLevel.innerHTML = 'normal' : false;
			level === 'hard' ? showLevel.innerHTML = 'difícil' : false;
		} else {
	...
	const createCloroqText = () => {
		...
		document.documentElement.lang === 'pt-BR' ? cloroqTxt.innerHTML = 'CLOROQUINA!' : cloroqTxt.innerHTML = 'CHLOROQUINE!';
	...
	const runGame = level => {
		...
		if (level === 'easy') speed = 1300;
		if (level === 'normal') speed = 1000;
		if (level === 'hard') speed = 800;
